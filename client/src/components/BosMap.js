...
const BosMap =()=>{
    ...
    //Calls bos311Service.hs function to get records for requested hex number and load it into regressionData state
    ...
    //Grab all records from collection and set to regressionData state
    const GetAllRegData = ()=>{
    ...
    //Request DB for records with selected user type and frequency
    ...
    //Event handler applied to every hexagon on map. On click it sets selectedHex state to selected hexagon, calls hexRegData on selected hex, and opens sidebar populated with obtained record
    ...
    return (
        <div>
            ...
            <Sidebar show={showSidebar} onHide={handleSidebarClose} selectedHex={SelectedHex} regressionData={regressionData}></Sidebar>
